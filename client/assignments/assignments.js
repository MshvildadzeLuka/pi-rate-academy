// =========================================================================
// FILE: assignments.js (Complete Implementation v5.5 - Final Fixes)
// =========================================================================
...
const uiRenderer = {
    ...
    renderTabs() {
        ...
        if (!isTeacherOrAdmin) {
            ...
            html += `<button class="tab-btn ${isReturnedActive ? 'active' : ''}" data-action="${ACTIONS.CHANGE_TAB}" data-tab="returned">Returned</button>`;
        }
        
        if (isTeacherOrAdmin) {
            const isRequestsActive = state.activeSubView === 'requests';
            html += `<button class="tab-btn ${isRequestsActive ? 'active' : ''}" data-action="${ACTIONS.CHANGE_TAB}" data-tab="requests">Requests</button>`;
        }
    ...
    renderListView() {
        ...
        if (state.studentAssignments.length === 0) {
            container.innerHTML = `<div class="empty-state">
                ...
                <p>No assignments to display in this view.</p>
            </div>`;
            return;
        }
    ...
    renderRetakeRequestsList() {
        ...
        if (!state.retakeRequests.length) {
            container.innerHTML = `<div class="empty-state">
                ...
                <p>No pending retake requests.</p>
            </div>`;
            return;
        }
        ...
        state.retakeRequests.forEach(request => {
            ...
            const assignmentTitle = assignment.templateTitle || 'Unknown Assignment';
                ...
                <div class="assignment-item" data-request-id="${request._id}">
                    ...
                    <div class="assignment-item-info">
                        ...
                        <span class="assignment-item-meta">
                            Student: ${student.firstName || ''} ${student.lastName || ''} (${student.email || ''})<br>
                            Reason: ${this.escapeHTML(request.reason)}<br>
                            Requested: ${new Date(request.createdAt).toLocaleString()}
                        </span>
                    </div>
                    <div class="assignment-item-status">
                        <button class="btn btn-primary" data-action="${ACTIONS.APPROVE_RETAKE}" data-request-id="${request._id}">Approve</button>
                        <button class="btn btn-danger" data-action="${ACTIONS.DENY_RETAKE}" data-request-id="${request._id}">Deny</button>
                    </div>
                </div>
            `;
        });
    ...
    groupAssignmentsByDate(assignments) {
        ...
        assignments.forEach(a => {
            ...
            let key = dueDate.toLocaleDateString(undefined, {
                month: 'long',
                day: 'numeric',
                year: 'numeric'
            });

            if (dueDate.toDateString() === today.toDateString()) key = 'Due Today';
            if (dueDate.toDateString() === yesterday.toDateString()) key = 'Due Yesterday';
        ...
        return `
            <div class="assignment-item" data-id="${assignment._id}" 
                ...
                <div class="assignment-item-info">
                    ...
                    <span class="assignment-item-meta">Due: ${dueDate.toLocaleString()}</span>
                </div>
                ${statusHTML}
    ...
    renderStudentDetailView() {
        ...
        template.querySelector('.due-date').textContent = dueDate.toLocaleString();
        template.querySelector('.points').textContent = assignment.templateId.points;
        template.querySelector('.instructions-text').innerHTML =
            this.sanitizeHTML(assignment.templateId.instructions || 'No instructions provided.');
        ...
        if (assignment.templateId.attachments?.length > 0) {
            attachmentsContainer.innerHTML = assignment.templateId.attachments.map(file => {
                ...
                <div class="file-list-item view-only">
                    ...
                    <div class="file-info">
                        <a href="${file.url}" target="_blank" data-action="${ACTIONS.VIEW_FILE}" data-url="${file.url}">
        ...
        if (assignment.submission?.files?.length > 0) {
            ...
            submissionFilesList.innerHTML = `<p>No files submitted yet.</p>`;
        }
    ...
    renderGradingPanel(studentAssignmentId) {
        ...
        if (!['completed', 'past-due'].includes(assignment.status)) {
            container.innerHTML = `<div class="grading-view-placeholder">
                ...
                <p>This assignment has not been submitted or is not yet due. Grading is not available.</p>
            </div>`;
            return;
        }

        let filesHTML = '<p>No files submitted.</p>';
        if (assignment.submission?.files?.length > 0) {
        ...
        container.innerHTML = `
            <h3>Submission from: ${this.escapeHTML(assignment.studentId.firstName)} ${this.escapeHTML(assignment.studentId.lastName)}</h3>
            ...
            <form class="grading-form" data-id="${assignment._id}">
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-primary">Return Grade</button>
                </div>
            </form>
        `;
        ...
        container.querySelector('.grading-form').addEventListener('submit', (e) => {
            ...
            if (isNaN(score) || score < 0 || score > assignment.templateId.points) {
                uiRenderer.showNotification(`Score must be between 0 and ${assignment.templateId.points}`, 'error');
                return;
            }
    ...
    openModal(type, data = null) {
        ...
        if (type === 'create-assignment') {
            ...
            if (data) {
                modalElement.querySelector('#modal-title').textContent = 'Edit Assignment';
    ...
    getStudentStatusText(assignment) {
        switch (assignment.status) {
            case STATUS.UPCOMING: return 'Not submitted';
            case STATUS.COMPLETED: return 'Turned In';
            case STATUS.GRADED: return `Graded: ${assignment.grade.score}/${assignment.templateId.points}`;
            case STATUS.PAST_DUE: return 'Missing';
    ...
    getTeacherStatusText(assignment) {
        switch (assignment.status) {
            case STATUS.COMPLETED: return 'Needs Grading';
            case STATUS.GRADED: return `Graded: ${assignment.grade.score}/${assignment.templateId.points}`;
            case STATUS.PAST_DUE: return 'Missing';
    ...
    handleFiles(files) {
        Array.from(files).forEach(file => {
            if (file.size > MAX_FILE_SIZE_MB * 1024 * 1024) {
                this.showNotification(`File ${file.name} exceeds ${MAX_FILE_SIZE_MB}MB limit`, 'error');
                return;
            }
...
const eventHandlers = {
            ...
            if (elements.createBtn) {
                elements.createBtn.addEventListener('click', () => {
                    if (!state.selectedGroupId && state.currentUser.role !== ROLES.ADMIN && state.groups.length > 0) {
                        uiRenderer.showNotification('Please select a group first to set a default.', 'warning');
                        return;
                    }
    ...
    handleGlobalClick(e) {
        ...
        const { action, id, fileName, url, containerId, requestId, templateId } = actionElement.dataset;
        ...
        switch (action) {
            ...
            case ACTIONS.VIEW_FILE: e.preventDefault(); this.handleViewFile(url); break;
    ...
    async handleCreateAssignment(form) {
        ...
        try {
            ...
            uiRenderer.showNotification('Assignment created successfully!', 'success');
            uiRenderer.closeModal();
            await this.loadStudentAssignments();
        } catch (error) {
            uiRenderer.showNotification(`Failed to create assignment: ${error.message}`, 'error');
        }
    },
    ...
    async handleUpdateAssignment(templateId, form) {
        ...
        try {
            ...
            uiRenderer.showNotification('Assignment updated successfully!', 'success');
            uiRenderer.closeModal();
            await this.loadStudentAssignments();
        } catch (error) {
            uiRenderer.showNotification(`Failed to update assignment: ${error.message}`, 'error');
        }
    },

    async handleDeleteAssignment(templateId) {
        if (confirm('Are you sure you want to permanently delete this assignment and all associated student submissions? This action cannot be undone.')) {
            try {
                await apiService.deleteAssignment(templateId);
                uiRenderer.showNotification('Assignment deleted successfully.', 'success');
                this.handleBackToList();
                await this.loadStudentAssignments();
            } catch (error) {
                uiRenderer.showNotification(`Failed to delete assignment: ${error.message}`, 'error');
            }
        }
    },
    ...
    async handleEditAssignment(templateId) {
        try {
            ...
            uiRenderer.showNotification('Failed to load assignment for editing.', 'error');
        }
    },
    ...
    async handleSubmitAssignment(assignmentId) {
        ...
        try {
            ...
            uiRenderer.showNotification('Assignment submitted successfully!', 'success');
            await this.loadStudentAssignments();
            this.handleBackToList();
        } catch (error) {
            uiRenderer.showNotification(`Failed to submit assignment: ${error.message}`, 'error');
        }
    },
    ...
    async handleUnsubmitAssignment(assignmentId) {
        try {
            ...
            uiRenderer.showNotification('Assignment unsubmitted successfully!', 'success');
            await this.loadStudentAssignments();
            this.handleBackToList();
        } catch (error) {
            uiRenderer.showNotification(`Failed to unsubmit assignment: ${error.message}`, 'error');
        }
    },

    async handleRequestRetake(assignmentId) {
        const reason = prompt('Please explain why you need a retake:');
        if (!reason) return;
        try {
            await apiService.createRetakeRequest({ requestableId: assignmentId, reason });
            uiRenderer.showNotification('Retake request submitted!', 'success');
        } catch (error) {
            uiRenderer.showNotification(`Failed to submit retake request: ${error.message}`, 'error');
        }
    },
    ...
    async handleGradeAssignment(assignmentId, data) {
        try {
            ...
            uiRenderer.showNotification('Assignment graded successfully!', 'success');
            await this.loadStudentAssignments();
        } catch (error) {
            uiRenderer.showNotification(`Failed to grade assignment: ${error.message}`, 'error');
        }
    },
    ...
    async loadStudentAssignments() {
        ...
        try {
            ...
            uiRenderer.showNotification('Failed to load assignments.', 'error');
            state.studentAssignments = [];
        }
    ...
    async loadRetakeRequests() {
        ...
        try {
            ...
            uiRenderer.showNotification('Failed to load retake requests.', 'error');
    ...
    handleApproveRetake(requestId) {
        ...
        form.onsubmit = async (e) => {
            ...
            if (!newDueDate) {
                uiRenderer.showNotification('Please select a new due date.', 'error');
                return;
            }
            try {
                await apiService.processRetakeRequest(requestId, STATUS.APPROVED, { newDueDate });
                uiRenderer.showNotification('Retake approved and assignment updated!', 'success');
                uiRenderer.closeModal();
                await this.loadRetakeRequests();
                await this.loadStudentAssignments();
            } catch (error) {
                uiRenderer.showNotification(`Failed to approve request: ${error.message}`, 'error');
            }
        };
    },
    ...
    async handleDenyRetake(requestId) {
        try {
            ...
            uiRenderer.showNotification('Retake request denied.', 'success');
            await this.loadRetakeRequests();
        } catch (error) {
            uiRenderer.showNotification('Failed to deny retake.', 'error');
        }
    },
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        elements.container.innerHTML = `
            ...
            <p>Could not load the assignments module. Please try again later.</p>
        `;
    }
});
