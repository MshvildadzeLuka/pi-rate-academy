  ...
  if (this.isModified('questions')) {
    try {
      ...
      if (!this.questions || this.questions.length === 0) {
        this.totalPoints = 0; // Set to 0 if no questions
        return next();
      }

      // We need to fetch the full question documents to get their points
      const questionDocs = await mongoose.model('Question').find({
        '_id': { $in: this.questions }
      });

      this.totalPoints = questionDocs.reduce((sum, question) => sum + (question.points || 0), 0);
    } catch (error) {
      return next(error);
    }
  }
